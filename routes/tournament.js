...
router.post('/setupKnockoutMatches', async (req, res) => {
    try {
        ...
        const knockoutTeams = topTeams.concat(thirdTeams.slice(0, powerOfTwo - topTeams.length));
        ...
        `, [knockoutMatches.map(match => [match.player1_id, match.player2_id, match.round, null, null])]);
